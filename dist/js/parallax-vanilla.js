      ...
      for (var i = 0; i < containers.length; i++) {
        ...
        var blocks = containers[i].getElementsByClassName(settings.block.class);
          ...
          for (var j = 0; j < blocks.length; j++) {
            ...
            var pvImages = block.el.getAttribute("pv-images") ? block.el.getAttribute("pv-images").split(" ") : [];

            if (pvImages.length == 0) {
              if (settings.block.image !== undefined) {
                block.el.style.backgroundImage = "url('" + settings.block.image + "')";
              }
            } else if (pvImages.length == 1) {
              block.el.style.backgroundImage = "url('" + pvImages[0] + "')";
            } else {
              var k = 0;
              block.el.style.backgroundImage = "url('" + pvImages[k] + "')";
              setInterval(function () {
                k = (k + 1) % pvImages.length;
                block.el.style.backgroundImage = "url('" + pvImages[k] + "')";
              }, 5000);
            }
